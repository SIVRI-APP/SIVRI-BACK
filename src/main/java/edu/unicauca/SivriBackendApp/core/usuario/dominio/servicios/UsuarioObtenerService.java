...
public class UsuarioObtenerService implements UsuarioObtenerCU {
     ...
     */
    @Override
    public Respuesta<Page<UsuarioListarConFiltroProyección>> listarConFiltro(int pageNo, int pageSize, String correo, TipoDocumento tipoDocumento, String numeroDocumento, String nombre, String apellido, TipoUsuario tipoUsuario, Integer organismoDeInvestigacionId) {
        Pageable pageable = PageRequest.of(pageNo, pageSize);

        Page<UsuarioListarConFiltroProyección> respuestaBd = usuarioObtenerREPO.listarConFiltro(pageable, correo, tipoDocumento, numeroDocumento, nombre, apellido, tipoUsuario, organismoDeInvestigacionId);
        return new RespuestaHandler<>(200, "ok", "", respuestaBd).getRespuesta();
    }

    @Override
    public Respuesta<UsuarioInformaciónDetalladaProyección> obtenerUsuarioInformaciónDetallada(long usuarioId) {
        UsuarioInformaciónDetalladaProyección respuestaBd = usuarioObtenerREPO.obtenerUsuarioInformaciónDetallada(usuarioId).orElseThrow(
